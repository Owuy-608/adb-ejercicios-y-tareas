...
JOIN 
  ...
  ON 
    p.id_pelicula = pa.id_pelicula 
WHERE 
    p.titulo = 'ACADEMY DINOSAUR';
...
JOIN 
    pelicula_actor AS pa 
  ON 
    a.id_actor = pa.id_actor
JOIN 
    pelicula_categoria AS pc 
  ON 
    pa.id_pelicula = pc.id_pelicula
JOIN 
    categoria AS c 
  ON 
...
JOIN 
    inventario AS i 
  ON 
    p.id_pelicula = i.id_pelicula 
WHERE 
    p.duracion IS NOT NULL 
GROUP BY 
    p.id_pelicula 
ORDER BY 
    p.duracion ASC 
